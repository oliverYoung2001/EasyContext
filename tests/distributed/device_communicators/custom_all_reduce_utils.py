...
from tests.distributed.device_communicators.cuda_wrapper import CudaRTLibrary
# from vllm.logger import init_logger
from tests.utils import (cuda_device_count_stateless,
...
def gpu_p2p_access_check(src: int, tgt: int) -> bool:
    ...
    if ((not is_distributed or get_world_group().local_rank == 0)
        ...
        # logger.info(
        print(f'[WARN]: '
            "generating GPU P2P access cache in %s", path)
        cache: Dict[str, bool] = {}
    ...
    # logger.info(
    print(f'[WARN]: '
        "reading GPU P2P access cache from %s", path)
